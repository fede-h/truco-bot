...
def _get_truco_rank(number: int, suit: Suit) -> int:
    if number == 1 and suit == Suit.ESPADAS:
        return 14
    if number == 1 and suit == Suit.BASTOS:
        return 13
    if number == 7 and suit == Suit.ESPADAS:
        return 12
    if number == 7 and suit == Suit.OROS:
        return 11
    if number == 3:
        return 10
    if number == 2:
        return 9
    if number == 1 and suit in (Suit.COPAS, Suit.OROS):
        return 8
    if number == 12:
        return 7
    if number == 11:
    ...
    if number == 10:
        return 5
    if number == 7 and suit in (Suit.COPAS, Suit.BASTOS):
        return 4
    if number == 6:
        return 3
    if number == 5:
...
ALL_CARDS = tuple(create_card(n, s) for s in Suit for n in (1, 2, 3, 4, 5, 6, 7, 10, 11, 12))
